...
Note that 9sh's VFS is not visible to UNIX processes. If you write `cat //vfs/file`, POSIX `cat` won't run; instead, a stream will be made from the virtual file contents. Stream-type unification, covered below, ensures that this process is consistent. Remote data is handled by either moving a process to the remote system (if possible), or by FIFO-streaming the data to the local one.
...
Note that these traits are unrelated to the types used to unify command alternatives -- the bridge is the `types` method of `VFS.Entry`, which provides a list of potential refined types for a given VFS node. A name might have multiple types because something like `foo/*` is a wildcard (expandable to a list of files), which we can see as a sequenced stream of filenames or as a sharded collection. Similarly, `cat foo.gz` might be a `(GZip Byte)` or just `Byte`. Beyond performance optimization, we prefer the resolution whose compressed entropy is maximized; this is the derivation that encodes the most information in types, which presumably is the most expressive.


## Stream typing
9sh will never reject a POSIX shell command due to a failed type unification. Its type system is used purely to infer latent intent and optimize pipelines. For example, let's consider something simple:
...
If `foo` is splittable, `grep bar` matches a low ratio of rows, and we have multiple processors, we might avoid a CPU bottleneck by parallelizing everything to the left of `sort`.
...
9sh connects local instances by default using the `~/.9sh.sock` domain socket. State is centralized per user to avoid race conditions and conflicts; that is, each user on each host has a single "server" 9sh process and clients connect to it. Every client perceives exactly the same VFS graph and persistent-object state.


## Multiplexing
In bash, you background a job with `&`. It then acquires a name like `%2` that refers to its PID. You `%2` to focus it, Ctrl+Z to background, and hope it doesn't print too much to stdout.
...
``` sh
$ bg @reverse rev     # run rev in background, call it @reverse
$ @reverse asdf       # send text to stdin (a common shorthand)
...
`bg` is a command defined by the standard VFS directory class; all of this machinery is part of the standard library rather than being hard-coded into the shell.

You can see that background jobs have flexible endpoints: you can change them as they're running. 9sh manages this for you by holding onto FIFOs or PTYs and splicing data to and from the endpoints. As in bash, background jobs are stopped if they attempt to change the terminal mode. These jobs must be explicitly foregrounded to interact with them.

9sh jobs are directory-scoped across shell instances, and since they're commands they can (1) provide custom parse continuations, and (2) participate in type unification. This enables them to modify their output format when writing to the terminal vs a file, for example.
