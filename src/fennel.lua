...
package.preload["fennel.binary"] = package.preload["fennel.binary"] or function(...)
  ...
  local _781_ = require("fennel.utils")
  local copy = _781_["copy"]
  local warn = _781_["warn"]
  ...
  local function execute(cmd)
    local _782_0 = os.execute(cmd)
    if (_782_0 == 0) then
      return true
    elseif (_782_0 == true) then
      return true
    end
  end
  ...
  local function module_name(open, rename, used_renames)
    ...
    do
      local _787_0 = rename[open]
      if (nil ~= _787_0) then
        local renamed = _787_0
  ...
  local function fennel__3ec(filename, native, options)
    ...
    local _791_
    do
      _791_ = "(do (local bundle_2_ ...) (fn loader_3_ [name_4_] (match (or (. bundle_2_ name_4_) (. bundle_2_ (.. name_4_ \".init\"))) (mod_5_ ? (= \"function\" (type mod_5_))) mod_5_ (mod_5_ ? (= \"string\" (type mod_5_))) (assert (if (= _VERSION \"Lua 5.1\") (loadstring mod_5_ name_4_) (load mod_5_ name_4_))) nil (values nil (: \"\n\\tmodule '%%s' not found in fennel bundle\" \"format\" name_4_)))) (table.insert (or package.loaders package.searchers) 2 loader_3_) ((assert (loader_3_ \"%s\")) ((or unpack table.unpack) arg)))"
    end
    fennel_loader = _791_:format(dotpath_noextension)
    local lua_loader = fennel["compile-string"](fennel_loader)
    local _792_ = options
    local rename_modules = _792_["rename-modules"]
  ...
  local function compile_binary(lua_c_path, executable_name, static_lua, lua_include_dir, native)
    ...
    local _794_
    do
      local _793_0 = shellout((cc .. " -dumpmachine"))
      if (nil ~= _793_0) then
        _794_ = _793_0:match("mingw")
      else
        _794_ = _793_0
      end
    end
    ...
    compile_command = {cc, "-Os", lua_c_path, table.concat(native, " "), static_lua, rdynamic, "-lm", _797_, "-o", (executable_name .. bin_extension), "-I", lua_include_dir, os.getenv("CC_OPTS")}
  ...
  local function native_path_3f(path)
    ...
    if (version_extension and (version_extension ~= "") and not version_extension:match("%.%d+")) then
      ...
      local _802_0 = extension
      if (_802_0 == "a") then
        return path
      elseif (_802_0 == "o") then
        return path
      elseif (_802_0 == "so") then
        return path
      elseif (_802_0 == "dylib") then
        return path
      else
        local _ = _802_0
        return false
  ...
  local function compile(filename, executable_name, static_lua, lua_include_dir, options, args)
    local _809_ = extract_native_args(args)
    local libraries = _809_["libraries"]
    local modules = _809_["modules"]
    local rename_modules = _809_["rename-modules"]
...
local function dosafely(f, ...)
  ...
  result = pack(xpcall(_752_, fennel.traceback))
...
local function handle_lua(i)
  ...
  local _757_0, _758_0 = os.execute(table.concat(cmd, " "))
  if (((_757_0 == true) and (_758_0 == "exit")) or (_757_0 == 0)) then
...
if not arg then return require("fennel") end
for i = #arg, 1, -1 do
  local _760_0 = arg[i]
  if (_760_0 == "--lua") then
    handle_lua(i)
...
do
  ...
  while (arg[i] and not options["ignore-options"]) do
    local _762_0 = arg[i]
    if (_762_0 == "--no-searcher") then
      options["no-searcher"] = true
      table.remove(arg, i)
    elseif (_762_0 == "--indent") then
      options.indent = table.remove(arg, (i + 1))
      if (options.indent == "false") then
        options.indent = false
      end
      table.remove(arg, i)
    elseif (_762_0 == "--add-package-path") then
      local entry = table.remove(arg, (i + 1))
      package.path = (entry .. ";" .. package.path)
      table.remove(arg, i)
    elseif (_762_0 == "--add-package-cpath") then
      local entry = table.remove(arg, (i + 1))
      package.cpath = (entry .. ";" .. package.cpath)
      table.remove(arg, i)
    elseif (_762_0 == "--add-fennel-path") then
      local entry = table.remove(arg, (i + 1))
      fennel.path = (entry .. ";" .. fennel.path)
      table.remove(arg, i)
    elseif (_762_0 == "--add-macro-path") then
      local entry = table.remove(arg, (i + 1))
      fennel["macro-path"] = (entry .. ";" .. fennel["macro-path"])
      table.remove(arg, i)
    elseif (_762_0 == "--load") then
      handle_load(i)
    elseif (_762_0 == "-l") then
      handle_load(i)
    elseif (_762_0 == "--no-fennelrc") then
      options.fennelrc = false
      table.remove(arg, i)
    elseif (_762_0 == "--correlate") then
      options.correlate = true
      table.remove(arg, i)
    elseif (_762_0 == "--check-unused-locals") then
      options.checkUnusedLocals = true
      table.remove(arg, i)
    elseif (_762_0 == "--globals") then
      allow_globals(table.remove(arg, (i + 1)), _G)
      table.remove(arg, i)
    elseif (_762_0 == "--globals-only") then
      allow_globals(table.remove(arg, (i + 1)), {})
      table.remove(arg, i)
    elseif (_762_0 == "--require-as-include") then
      options.requireAsInclude = true
      table.remove(arg, i)
    elseif (_762_0 == "--assert-as-repl") then
      options.assertAsRepl = true
      table.remove(arg, i)
    elseif (_762_0 == "--skip-include") then
      ...
      table.remove(arg, i)
    elseif (_762_0 == "--use-bit-lib") then
      options.useBitLib = true
      table.remove(arg, i)
    elseif (_762_0 == "--metadata") then
      options.useMetadata = true
      table.remove(arg, i)
    elseif (_762_0 == "--no-metadata") then
      options.useMetadata = false
      table.remove(arg, i)
    elseif (_762_0 == "--no-compiler-sandbox") then
      options["compiler-env"] = _G
      table.remove(arg, i)
    elseif (_762_0 == "--raw-errors") then
      options.unfriendly = true
      table.remove(arg, i)
    elseif (_762_0 == "--plugin") then
...
local function eval(form)
  local _772_
  if (form == "-") then
    _772_ = (io.stdin):read("*a")
  else
    _772_ = form
  end
  return print(dosafely(fennel.eval, _772_, options))
end
local function compile(files)
  for _, filename in ipairs(files) do
    ...
    do
      local _775_0, _776_0 = nil, nil
      local function _777_()
        return fennel["compile-string"](f:read("*a"), options)
      end
      _775_0, _776_0 = xpcall(_777_, fennel.traceback)
      if ((_775_0 == true) and (nil ~= _776_0)) then
        local val = _776_0
        print(val)
      elseif (true and (nil ~= _776_0)) then
        local _0 = _775_0
        local msg = _776_0
...
if ((_G.type(_779_0) == "table") and _780_(...)) then
  return repl()
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--repl")) then
  return repl()
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--compile")) then
  local files = {select(2, (table.unpack or _G.unpack)(_779_0))}
  return compile(files)
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "-c")) then
  local files = {select(2, (table.unpack or _G.unpack)(_779_0))}
  return compile(files)
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--compile-binary") and (nil ~= _779_0[2]) and (nil ~= _779_0[3]) and (nil ~= _779_0[4]) and (nil ~= _779_0[5])) then
  local filename = _779_0[2]
  local out = _779_0[3]
  local static_lua = _779_0[4]
  local lua_include_dir = _779_0[5]
  local args = {select(6, (table.unpack or _G.unpack)(_779_0))}
  local bin = require("fennel.binary")
  options.filename = filename
  options.requireAsInclude = true
  return bin.compile(filename, out, static_lua, lua_include_dir, options, args)
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--compile-binary")) then
  local cmd = (arg[0] or "fennel")
  return print((require("fennel.binary").help):format(cmd, cmd, cmd))
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--eval") and (nil ~= _779_0[2])) then
  local form = _779_0[2]
  return eval(form)
elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "-e") and (nil ~= _779_0[2])) then
  local form = _779_0[2]
  return eval(form)
else
  ...
  elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "--help")) then
    return print(help)
  elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "-h")) then
    return print(help)
  elseif ((_G.type(_779_0) == "table") and (_779_0[1] == "-")) then
    return dosafely(fennel.eval, (io.stdin):read("*a"))
  elseif ((_G.type(_779_0) == "table") and (nil ~= _779_0[1])) then
    local filename = _779_0[1]
    local args = {select(2, (table.unpack or _G.unpack)(_779_0))}
    arg[-2] = arg[-1]
    arg[-1] = arg[0]
